...
// ============================================================================
// POST /api/v1/pipeline - Create new tender card
// ============================================================================
router.post('/', (req, res) => {
  try {
...
// ============================================================================
// POST /api/v1/pipeline/from-scanner - Add tender from scanner feed
// ============================================================================
router.post('/from-scanner', (req, res) => {
  try {
    ...
    // Check for duplicates by tender_no
    ...
    db.prepare(`
      INSERT INTO bpo_tender_lifecycle (
        id, source_type, source_id, tender_no, title, agency, category,
        published_date, closing_date, estimated_value, stage, priority,
        external_url, assigned_to
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, 'gebiz_active', activeTender.id, activeTender.tender_no,
      activeTender.title, activeTender.agency, activeTender.category,
      activeTender.published_date, activeTender.closing_date,
      activeTender.estimated_value, stage, priority,
      activeTender.url, assigned_to || null
    );

    // Mark as added in active tenders
...
// IMPORTANT: This MUST be before /:id route to avoid matching "stats" as :id
// ============================================================================
...
// ============================================================================
// GET /api/v1/pipeline/deadlines - Get tenders closing soon
// IMPORTANT: This MUST be before /:id route to avoid matching "deadlines" as :id
// ============================================================================
...
// ============================================================================
// GET /api/v1/pipeline/competitor-intel/:agency - Get competitor data for agency
// ============================================================================
router.get('/competitor-intel/:agency', (req, res) => {
  try {
    ...
    const ourHistory = db.prepare(`
      SELECT
        COUNT(*) as total_bids,
        SUM(CASE WHEN outcome = 'won' THEN 1 ELSE 0 END) as our_wins,
        SUM(CASE WHEN outcome = 'lost' THEN 1 ELSE 0 END) as our_losses,
        SUM(CASE WHEN outcome = 'won' THEN actual_contract_value ELSE 0 END) as our_won_value
      FROM bpo_tender_lifecycle
      WHERE agency = ?
    `).get(agency);
    db.close();
    ...
    res.json({
      ...
      data: {
        agency,
        competitors,
        our_history: ourHistory || { total_bids: 0, our_wins: 0, our_losses: 0, our_won_value: 0 }
      }
    });
  } catch (error) {
...
// ============================================================================
// GET /api/v1/pipeline/:id - Get single tender
// IMPORTANT: This MUST be after /stats and /deadlines routes
// ============================================================================
...
// ============================================================================
// PATCH /api/v1/pipeline/:id - Update tender
// ============================================================================
router.patch('/:id', (req, res) => {
  try {
    ...
    const allowedFields = [
      ...
      'stage', 'priority', 'win_probability', 'is_urgent', 'is_featured', 'assigned_to',
...
// ============================================================================
// DELETE /api/v1/pipeline/:id - Delete tender
// ============================================================================
...
// ============================================================================
// POST /api/v1/pipeline/:id/move - Move tender to different stage
// ============================================================================
...
// ============================================================================
// POST /api/v1/pipeline/:id/decision - Record Go/No-Go decision
// ============================================================================
...
// ============================================================================
// POST /api/v1/pipeline/renewal/:renewalId/move - Move renewal to opportunity
// ============================================================================
