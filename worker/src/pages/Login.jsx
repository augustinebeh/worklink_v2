...
import TelegramLoginButton from '../components/auth/TelegramLogin';
  ...
  useEffect(() => {
    ...
    if (refCode) {
      ...
      fetch(`/api/v1/referrals/validate/${refCode}`)
        ...
        .catch(e => logger.error('Referral validation error:', e));
    }
  }, [location.search]);
    ...
    if (hash && hash.includes('tgAuthResult=')) {
      try {
        ...
        if (hasRequiredFields) {
          ...
          logger.error('Invalid Telegram auth data - missing required fields:', requiredFields.filter(field => !(field in telegramUser)));
          setError('Invalid authentication data received from Telegram');
        }
      } catch (e) {
        logger.error('Failed to parse Telegram auth data from URL:', e);
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center px-6 py-8 pt-safe">
        ...
        <div className="w-full max-w-sm">
          ...
          <EmailLogin
            ...
            onToggleShow={() => setShowEmailLogin(!showEmailLogin)}
            touched={touched}
