 ...
 *
 * @module db/utils/table-creator
 */

const { db } = require('..');
const { createLogger } = require('../../utils/structured-logger');
const logger = createLogger('table-creator');
...
function ensureTableExists(tableName, createTableSQL) {
  try {
    ...
    logger.error(`Failed to ensure table ${tableName}`, { error: error.message });
    return false;
  }
}
...
function ensureTableExistsWithFallback(tableName) {
  ...
  if (!tableExists) {
    logger.warn(`Table ${tableName} missing - attempting to create...`);
