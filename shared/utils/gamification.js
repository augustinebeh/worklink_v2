...
const XP_THRESHOLDS = generateXPThresholds(100);
 ...
 */
function getTierBenefits(level) {
...
/**
 * Format XP number with commas or K/M suffix
 */
 ...
 */
function calculateJobXP(hours, isUrgent = false, wasOnTime = false, rating = null) {
 ...
 */
function hasTierBenefit(level, benefit) {
 ...
 */
function getJobVisibilityDelay(level) {
  const tier = getLevelTier(level);
...
/**
 * Get XP for next level (alias for backward compatibility)
 */
